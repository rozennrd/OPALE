...
def generate_schedule(data: RequestData) -> List[CalendrierOutput]:
    ...
    #Verif des jours travaillés par promo
    ...
    # Contraintes : Ne pas dépasser 8h de cours au total par promo par jour
    for promo, jours_travailles in promo_calendar_info.items():
        for jour_index in range(len(jours_travailles)):
            total_heures_promo = sum(
                heures_par_jour[f"heures_{promo}_{course['cours']}_jour_{jour_index}"]
                for course in promo_courses_info[promo]
            )
            model.Add(total_heures_promo <= 8)  
